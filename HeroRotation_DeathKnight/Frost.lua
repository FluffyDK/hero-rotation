...
local function Standard()
  ...
  if S.FrostStrike:IsReadyP() and (Player:BuffRemainsP(S.IcyTalonsBuff) <= Player:GCD() and Player:BuffP(S.IcyTalonsBuff) and (not S.BreathofSindragosa:IsAvailable() or S.BreathofSindragosa:CooldownRemainsP() > 15)) then
  ...
  if S.FrostStrike:IsReadyP() and (S.RemorselessWinter:CooldownRemainsP() <= (2 * Player:GCD()) and S.GatheringStorm:IsAvailable()) then
    if HR.Cast(S.FrostStrike) then return ""; end
  end
  ...
  if S.FrostStrike:IsReadyP() and (Player:RunicPowerDeficit() < (15 + (S.RunicAttenuation:IsAvailable() and 1 or 0) * 3)) then
    if HR.Cast(S.FrostStrike) then return ""; end
  end
  ...
  -- frost_strike
  if S.FrostStrike:IsReadyP() then
    if HR.Cast(S.FrostStrike) then return ""; end
  end
  -- horn_of_winter
...
local function BoS_Pooling()
  ...
  if S.FrostStrike:IsReadyP() and (S.PillarOfFrost:CooldownRemainsP() > Player:RuneTimeToX(4) and Player:RunicPowerDeficit() < 40) then
    if HR.Cast(S.FrostStrike) then return ""; end
  end
  return false;
end
...
local function Obliteration()
  ...
  if S.HowlingBlast:IsCastableP() and (Player:Buff(S.Rime) and Cache.EnemiesCount[10] >= 2) then
    if HR.Cast(S.HowlingBlast) then return ""; end
  end
  -- frost_strike,if=!buff.rime.up|runic_power.deficit<10|rune.time_to_2>gcd
  if S.FrostStrike:IsReadyP() and not Player:Buff(S.Rime) or Player:RunicPowerDeficit() < 10 or Player:RuneTimeToX(2) > Player:GCD() then
    if HR.Cast(S.FrostStrike) then return ""; end
  end
  -- howling_blast,if=buff.rime.up
  if S.HowlingBlast:IsCastableP() and (Player:Buff(S.Rime)) then
...
local function Cooldowns()
  ...
  if Settings.DeathKnight.Commons.UsePotions and I.BattlePotionofStrength:IsReady() and Player:Buff(S.PillarOfFrost) and Player:Buff(S.EmpowerRuneWeapon) then
...
local function APL()
  ...
  if not Player:AffectingCombat() then
    ...
    -- Volley toggle
    -- Opener
    if S.HowlingBlast:IsCastableP() and Everyone.TargetIsValid() and not Target:Debuff(S.FrostFever) then
      if HR.Cast(S.HowlingBlast) then return ""; end
    end
  end
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.HowlingBlast:IsCastableP() and not Target:Debuff(S.FrostFever) and (not S.BreathofSindragosa:IsAvailable() or S.BreathofSindragosa:CooldownRemainsP() > 15) then
      if HR.Cast(S.HowlingBlast) then return ""; end
    end
    ...
    if S.FrostStrike:IsReadyP() and S.IcyTalons:IsAvailable() and Player:BuffRemainsP(S.IcyTalonsBuff) <= Player:GCD() then
      if HR.Cast(S.FrostStrike) then return ""; end
    end
