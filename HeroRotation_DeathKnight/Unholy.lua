...
local ShouldReturn; -- Used for the return string

--- ===== APL =====
--- ===============
local function AOE()
  -- death_and_decay,if=cooldown.apocalypse.remains
  if S.DeathAndDecay:IsCastableP() and S.Apocalypse:CooldownDown() then
  ...
  if S.Epidemic:IsReadyP() and Player:Buff(S.DeathAndDecayBuff) and Player:Runes() < 2 and not PoolingForGargoyle() then
    if HR.Cast(S.Epidemic) then return ""; end
  end
  -- death_coil,if=death_and_decay.ticking&rune<2&!variable.pooling_for_gargoyle
  if S.DeathCoil.IsReadyP() and Player:Buff(S.DeathAndDecayBuff) and Player:Runes() < 2 and not PoolingForGargoyle() then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  -- scourge_strike,if=death_and_decay.ticking&cooldown.apocalypse.remains
  if S.ScourgeStrike:IsCastableP() and Player:Buff(S.DeathAndDecayBuff) and S.Apocalypse:CooldownDown() then
    if HR.Cast(S.ScourgeStrike) then return ""; end
  end
  -- clawing_shadows,if=death_and_decay.ticking&cooldown.apocalypse.remains
  if S.ClawingShadows:IsCastableP() and Player:Buff(S.DeathAndDecayBuff) and S.Apocalypse:CooldownDown() then
    if HR.Cast(S.ClawingShadows) then return ""; end
  end
  -- epidemic,if=!variable.pooling_for_gargoyle
  if S.Epidemic:IsReadyP() and not PoolingForGargoyle() then
    if HR.Cast(S.Epidemic) then return ""; end
  end
  -- festering_strike,target_if=debuff.festering_wound.stack<=1&cooldown.death_and_decay.remains
  if S.FesteringStrike:IsCastableP() and Target:DebuffStack(S.FesteringWound) <= 1 and S.DeathAndDecay:CooldownDown() then
  ...
  if S.FesteringStrike:IsCastableP() and (S.BurstingSores:IsAvailable() and Cache.EnemiesCount[8] >= 2 and Target:DebuffStack(S.FesteringWound) <= 1) then
    if HR.Cast(S.FesteringStrike) then return ""; end
  end
  -- death_coil,if=buff.sudden_doom.react&rune.deficit>=4
  if S.DeathCoil.IsReadyP() and Player:Buff(S.SuddenDoom) and Player:Runes() <= 2 then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  -- death_coil,if=buff.sudden_doom.react&!variable.pooling_for_gargoyle|pet.gargoyle.active
  if S.DeathCoil.IsReadyP() and Player:Buff(S.SuddenDoom) and not PoolingForGargoyle() or S.SummonGargoyle:TimeSinceLastCast() <= 22 then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  -- death_coil,if=runic_power.deficit<14&(cooldown.apocalypse.remains>5|debuff.festering_wound.stack>4)&!variable.pooling_for_gargoyle
  if S.DeathCoil.IsReadyP() and Player:RunicPowerDeficit() < 14 and (S.Apocalypse:CooldownRemainsP() > 5 or Target:DebuffStackP(S.FesteringWound) > 4) and not PoolingForGargoyle() then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  ...
  if S.ScourgeStrike:IsCastableP() and (((Target:Debuff(S.FesteringWound) and S.Apocalypse:CooldownRemainsP() > 5) or Target:DebuffStack(S.FesteringWound) > 4) and S.ArmyOfTheDead:CooldownRemainsP() > 5 or S.ArmyOfTheDead:IsCastableP()) then
    if HR.Cast(S.ScourgeStrike) then return ""; end
  end
  ...
  if S.ClawingShadows:IsCastableP() and (((Target:Debuff(S.FesteringWound) and S.Apocalypse:CooldownRemainsP() > 5) or Target:DebuffStack(S.FesteringWound) > 4) and S.ArmyOfTheDead:CooldownRemainsP() > 5 or S.ArmyOfTheDead:IsCastableP()) then
    if HR.Cast(S.ClawingShadows) then return ""; end
  end
  -- death_coil,if=runic_power.deficit<20&!variable.pooling_for_gargoyle
  if S.DeathCoil.IsReadyP() and Player:RunicPowerDeficit() < 20 and not PoolingForGargoyle() then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  ...
  if S.FesteringStrike:IsCastableP() and (((((Target:DebuffStack(S.FesteringWound) < 4 and not Player:Buff(S.UnholyFrenzy)) or Target:DebuffStack(S.FesteringWound) < 3) and S.Apocalypse:CooldownRemainsP() < 3) or Target:DebuffStack(S.FesteringWound) < 1) and S.ArmyOfTheDead:CooldownRemainsP() > 5 or S.ArmyOfTheDead:IsCastableP()) then
    if HR.Cast(S.FesteringStrike) then return ""; end
  end
  -- death_coil,if=!variable.pooling_for_gargoyle
  if S.DeathCoil.IsReadyP() and not PoolingForGargoyle() then
    if HR.Cast(S.DeathCoil) then return ""; end
  end
  return false;
end
...
local function Generic()
  ...
  if S.DeathAndDecay:IsCastableP() and S.Pestilence:IsAvailable() and S.Apocalypse:CooldownDown() then
    if HR.Cast(S.DeathAndDecay) then return ""; end
  end
  -- defile,if=cooldown.apocalypse.remains
  if S.Defile:IsCastableP() and S.Apocalypse:CooldownDown() then
    if HR.Cast(S.Defile) then return ""; end
  end
  -- scourge_strike,if=((debuff.festering_wound.up&cooldown.apocalypse.remains>5)|debuff.festering_wound.stack>4)&cooldown.army_of_the_dead.remains>5
  if S.ScourgeStrike:IsCastableP() and (((Target:Debuff(S.FesteringWound) and S.Apocalypse:CooldownRemainsP() > 5) or Target:DebuffStack(S.FesteringWound) > 4) and (S.ArmyOfTheDead:CooldownRemainsP() > 5 or S.ArmyOfTheDead:IsCastableP())) then
...
local function Cooldowns()
  ...
  -- dark_transformation
  if S.DarkTransformation:IsCastableP() and Pet:IsActive() then
  ...
  if S.UnholyFrenzy:IsCastableP() and Target:DebuffStack(S.FesteringWound) < 4 then
    if HR.Cast(S.UnholyFrenzy, Settings.Unholy.GCDasOffGCD.UnholyFrenzy) then return ""; end
  end
  -- unholy_frenzy,if=active_enemies>=2&((cooldown.death_and_decay.remains<=gcd&!talent.defile.enabled)|(cooldown.defile.remains<=gcd&talent.defile.enabled))
  if S.UnholyFrenzy:IsCastableP() and Cache.EnemiesCount[10] >= 2 and ((S.DeathAndDecay:CooldownRemainsP() <= Player:GCD() and not S.Defile:IsAvailable()) or (S.Defile:CooldownRemainsP() <= Player:GCD() and S.Defile:IsAvailable())) then
    if HR.Cast(S.UnholyFrenzy, Settings.Unholy.GCDasOffGCD.UnholyFrenzy) then return ""; end
  end
...
local function APL()
  ...
  if not Player:AffectingCombat() then
    ...
    if not Pet:IsActive() and S.SummonPet:IsCastableP() then
    ...
    if Settings.DeathKnight.Commons.UsePotions and I.BattlePotionofStrength:IsReady() then
      if HR.CastLeft(I.BattlePotionofStrength) then return ""; end
    end
    -- Opener
    -- outbreak if virulent_plague is not  the target and we are not in combat
    if S.Outbreak:IsCastableP() and Everyone.TargetIsValid() and not Target:Debuff(S.VirulentPlagueDebuff) then
  ...
  if S.Outbreak:IsCastableP() and not Target:Debuff(S.VirulentPlagueDebuff) or Target:DebuffRemainsP(S.VirulentPlagueDebuff) < Player:GCD() * 1.5 then
    if HR.Cast(S.Outbreak) then return ""; end
  end
  -- apocalypse,if=debuff.festering_wound.stack>=4
  if S.Apocalypse:IsCastableP() and Target:DebuffStack(S.FesteringWound) >= 4 then
